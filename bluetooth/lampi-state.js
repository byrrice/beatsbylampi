...
function LampiState() {
    ...
    this.song = "Ghost Town By Kanye West";
    ...
    mqtt_client.on('message', function(topic, message) {
        ...
        if (that.song != new_song) {
            console.log('MQTT - NEW SONG');
            that.song = new_song;
            that.emit('changed-song', that.song);
        }
...
LampiState.prototype.set_song = function(song) {
    this.song = song;
    var tmp = {'song': this.song };
    this.mqtt_client.publish('lamp/set_config', JSON.stringify(tmp));
